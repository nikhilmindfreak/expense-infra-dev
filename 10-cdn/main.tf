#we mix and matched cloud front with load balncer

resource "aws_cloudfront_distribution" "web_cdn" {
  origin {
    domain_name              =  "web-${var.environment}.${var.zone_name}"#web-dev.devospme.online
  ...
  aliases = ["web-${var.common_tags.Component}.${var.zone_name}"]#web-cdn.devopme.online
...
module "records" {
  ...
  records = [
    {
      name    = "web-cdn" #web-cdn.devopsme.online
      type    = "A"
